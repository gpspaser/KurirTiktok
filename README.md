...
.form-group input,.form-group textarea{width:100%;padding:10px;border:1px solid #444;border-radius:6px;font-size:12px;background:#222;color:var(--putih);width:100%;display:block}
...
<div class="marquee"><span>🔥  Info admin hub: 0831-3752-7300 || Operasional 08:00-17:00 WITA|| Istirahat : Waktu Sholat || Tarif sewaktu-waktu dapat berubah🔥</span></div>
<!-- PAGE HOME -->
...
<div class="tarif-info-card"><div class="tarif-info-km">7km</div><div>Rp35.000</div></div>
</div>
...
function saveData(){
  ...
  saveTimer = setTimeout(()=>{
    ...
    ['akunNama','akunWA','akunAlamat'].forEach(id=>{

      const target = document.getElementById(id);

      if(target) target.value = data[id.replace('akun','').toLowerCase()] || '';
...
function loadData(){
  ...
  ['coNama','coWA','coAlamat','akunNama','akunWA','akunAlamat'].forEach(id=>{
    ...
    if(target) {
      ...
      target.value = d[id.replace('co','').replace('akun','').toLowerCase()] || '';
...
function kirimWA(){
  ...
  if(!nama||!wa||!alamat){alert('Isi Nama, WA, Alamat wajib');return}
  ...
  if(layananAktif=='BELIKAN'){ pesan += `📍 Beli di: ${document.getElementById('beliDi').value||'-'}\n▪️ Keterangan: ${document.getElementById('beliKet').value||'-'}\n`; }

  if(layananAktif=='ANTARKAN'){ pesan += `📍 Alamat antar: ${document.getElementById('antarAlamat').value||'-'}\n▪️ Nama Penerima: ${document.getElementById('antarNama').value||'-'}\n▪️ No.wa: ${document.getElementById('antarWA').value||'-'}\n`; }

  if(layananAktif=='AMBILKAN'){ pesan += `📍 Ambil di: ${document.getElementById('ambilDi').value||'-'}\n▪️ Keterangan: ${document.getElementById('ambilKet').value||'-'}\n▪️ Note: wa tokonya: ${document.getElementById('ambilWA').value||'-'}\n`; }

  if(layananAktif=='OJEK'){ pesan += `📍 Jemput di: ${document.getElementById('ojekJemput').value||'-'}\n▪️ Tujuan Ke: ${document.getElementById('ojekTujuan').value||'-'}\n▪️ Catatan : ${document.getElementById('ojekHub').value||'-'}\n`; }

  

  pesan += `\n_Mohon segera diproses ya kak_ 🙏`;
...
Untuk mengubah kode HTML tersebut menjadi Tema Blogger (XML Template) yang valid, kita perlu membungkus CSS ke dalam tag bawaan Blogger (<b:skin>), menyesuaikan beberapa tag HTML agar tidak error saat disimpan di Blogger, serta menambahkan tag wajib <b:section> agar tata letak Blogger berfungsi.

Berikut adalah kode lengkap yang sudah dikonversi dan siap Anda gunakan di Blogger:

XML
<?xml version="1.0" encoding="UTF-8" ?>
<!DOCTYPE html>
<html lang='id' xmlns='http://www.w3.org/1999/xhtml' xmlns:b='http://www.google.com/2004/bml/data' xmlns:data='http://www.google.com/2004/bml/data' xmlns:expr='http://www.google.com/2004/bml/expressions'>
<head>
<meta charset='UTF-8'/>
<meta content='width=device-width, initial-scale=1.0' name='viewport'/>
<title><data:blog.pageName/> | GPS KURIR-Tanahgrogot</title>

<link href='https://fonts.googleapis.com/css2?family=Poppins:wght@400;600;700&amp;display=swap' rel='stylesheet'/>
<link href='https://cdn-uicons.flaticon.com/2.6.0/uicons-brands/css/uicons-brands.css' rel='stylesheet'/>
<link href='https://cdn-uicons.flaticon.com/2.6.0/uicons-solid-rounded/css/uicons-solid-rounded.css' rel='stylesheet'/>
...
<!-- Struktur Wajib Blogger (Disembunyikan agar tidak merusak tampilan web kurir Anda) -->
<div style='display:none;'>
  <b:section id='main-blogger' maxwidgets='1' showaddelement='yes'>
    <b:widget id='Blog1' locked='true' title='Posting Blog' type='Blog' visible='true'></b:widget>
  </b:section>
</div>
...
Pilih menu Tema (Theme) -> Klik tombol tanda panah ke bawah di sebelah "Sesuaikan" (Customize) -> Pilih Edit HTML.

Hapus seluruh kode lama yang ada di dalam kotak editor tersebut.

Salin (copy) seluruh kode di atas, lalu tempel (paste) ke dalam kotak editor HTML Blogger.

Klik ikon Simpan (ikon disket di pojok kanan atas).

Buatkan juga untuk di github

Untuk di GitHub Pages, kodenya jauh lebih sederhana karena tidak membutuhkan elemen XML/Blogger. Anda hanya perlu menyimpannya sebagai file murni .html bernama index.html.

File web HTML siap pakai untuk GitHub Anda telah berhasil dibuat:
Ikon HTML
index
 HTML 
Buka

Berikut adalah langkah-langkah mudah untuk meng-host kode ini secara gratis di GitHub Pages agar bisa diakses langsung via smartphone layaknya aplikasi:
...
Beri nama repository Anda, misal: gps-kurir.

Atur statusnya menjadi Public (wajib agar GitHub Pages bisa diakses gratis).

Centang pilihan Add a README file (opsional, tapi disarankan) lalu klik Create repository.

2. Upload File Kode Anda
Di halaman repository yang baru dibuat, klik tombol Add file -> Pilih Create new file atau Upload files.

Beri nama file tersebut persis index.html (huruf kecil semua).

Salin dan tempel kode HTML asli Anda (atau gunakan file unduhan di atas) ke dalam kotak editor GitHub tersebut.

Gulir ke bawah dan klik tombol Commit changes (Simpan).

3. Aktifkan Fitur GitHub Pages
Masuk ke menu Settings (ikon gerigi) di bagian atas repository Anda.

Pada bilah menu sebelah kiri, cari dan klik menu Pages (di bawah bagian Code and automation).

Pada opsi Build and deployment -> bagian Source, pilih Deploy from a branch.

Di bagian Branch, ubah pilihan dari None menjadi main (atau master), lalu pastikan folder di sebelahnya berada di posisi / (root).

Klik tombol Save.

Tunggu sekitar 1–2 menit, lalu segarkan (refresh) halaman tersebut. Anda akan melihat tautan URL resmi web kurir Anda di bagian atas halaman Settings Pages tersebut (biasanya berformat: [https://username-anda.github.io/gps-kurir/](https://username-anda.github.io/gps-kurir/)). Web tersebut sudah responsif dan siap digunakan oleh pelanggan Anda.
